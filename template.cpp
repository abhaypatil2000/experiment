#include<bits/stdc++.h>
//template

#define io ios_base::sync_with_stdio(0); cin.tie(NULL); cout.tie(NULL)
#define fo(i, x) for (i = 0; i < x; i++)
#define foi(x) for (i = 0; i < x; i++)
#define foj(x) for (j = 0; j < x; j++)
#define fok(x) for (k = 0; k < x; k++)
#define Fo(i, a, b) for (i = a; i < b; i++)
#define Foi(a, b) for (i = a; i < b; i++)
#define Foj(a, b) for (j = a; j < b; j++)
#define Fok(a, b) for (k = a; k < b; k++)
#define pb(x) push_back(x)
#define pf(x) push_front(x)
#define mp(x, y) make_pair(x, y)
#define unmap unordered_map
#define unmapi unordered_map<int, int>
#define vi vector<int>
#define ff first
#define ss second
#define ggg << " "
#define pii pair<int, int>
#define mii map<int, int>
#define kl cout << endl
#define present(a, b) (a.find(b) != a.end())
#define ub upper_bound
#define lb lower_bound
#define sot(a) sort(a.begin(), a.end())
#define sbsot(a) stable_sort(a.begin(), a.end())
#define fox(a) for (auto x : a)
#define foy(a, b) for (auto a : b)
#define ll long long int
#define deb(x) cout << #x << " " << x << endl;
#define fills(a, b) for (auto ploopy = a.begin(); ploopy != a.end(); ploopy++) *ploopy = b;
#define fill(a,v) memset(a, v, sizeof (a))
#define fill(a, b) for (int ploopy = 0; ploopy < sizeof(a)/sizeof(a[0]); ploopy++) a[ploopy] = b;
using namespace std;
ll mox = 1e9 + 7;

int main()
{
	//template
}
